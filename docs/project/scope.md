...
- deterministic Vulkan device/queue/surface/swapchain policy, synchronization2
  transfer clear, per-image presentation semaphores, zero-extent/surface-change
  deferral, transactional recreation, and optional/required validation;
...
types. A successful cross-technology claim currently means that the exact same
linear `DiagnosticFrame` reaches both backends and each successfully presents;
it is not a screenshot/pixel-equality claim. Native frame failures are terminal
for that renderer object. Automated runs require a real presentation.
...
- `.gexworld` loading or terrain/validity visualisation in game or editor;
...
`v0.1.7` should be the smallest shared terrain-view proof. Before implementation,
agree only the resource/shader/camera/extraction contracts required to load the
existing synthetic `.gexworld` package through `GameEX::WorldFormat` and show
height plus validity in the standalone world editor under both backends. Decide
the shader source/build/reflection approach at that gate; it was intentionally
unnecessary for the transfer-clear slice.

The viewer must inspect one layer before any combination and must not imply that
synthetic data is real terrain. A separate decision remains necessary before:
